...
- Streams fed by an inbound publisher (for example, tablet WebRTC publishers)
  ...
  at it. Frigate only ever dials its own camera name. A publisher that connects
  while privacy is on lands on the clip stream; switching off then falls back
  to the coordinated restart for that apply.
