...
class LiveSwapTests(unittest.TestCase):
    ...
    def test_restore_restarts_when_the_parked_name_is_gone_or_a_publisher_appeared(
        self,
    ):
        swaps = {"tablet": {"sources": ["ffmpeg:tablet"], "cameras": ["tablet"]}}
        # the alias was recorded but go2rtc lost it (Frigate restarted while private)
        job = self._job()
        job.client.go2rtc_streams.return_value = {
            "tablet": {"producers": [{"url": "ffmpeg:/t.mp4"}]}
        }
        ...
        job.client.go2rtc_put_stream.assert_not_called()
        job.client.restart_api.assert_called_once_with()
        # nothing parked, but a publisher attached itself to the clip while private
        job = self._job()
        job.client.go2rtc_streams.return_value = {
            "tablet": {
